...
import (
	"context"
	"github.com/networkservicemesh/cmd-nse-vl3-vpp/chain/prefixpool"
...
func NewServer(prefixes, ownPrefixes, excludePrefixes []string) networkservice.NetworkServiceServer {
	return &cidrServer{
		prefixes: prefixes,
		ownPrefixes: ownPrefixes,
...
func (c *cidrServer) init() {
	...
	if _, err = c.pool.ExcludePrefixes(c.ownPrefixes); err != nil {
		c.initErr = errors.New("unable to exclude own prefixes")
		return
	}
...
func (c *cidrServer) Request(ctx context.Context, request *networkservice.NetworkServiceRequest) (*networkservice.Connection, error) {
	...
	ipContext = conn.GetContext().GetIpContext()
	if _, ok := load(ctx, metadata.IsClient(c)); !ok {
		/*  Absence of ExtraPrefixes means, that this request comes form ordinal client and  */
		/*  we need to set SrcRoute = all prefixes that are available through this element   */
		if len(ipContext.GetExtraPrefixes()) == 0 {
			...
			/*  Otherwise this request comes form vl3-endpoint and it is enough to set only ownPrefixes  */
			for i := 0; i < len(c.ownPrefixes); i++ {
				ipContext.SrcRoutes = append(ipContext.SrcRoutes, &networkservice.Route{
					Prefix: c.ownPrefixes[i],
				})
			}
		}
