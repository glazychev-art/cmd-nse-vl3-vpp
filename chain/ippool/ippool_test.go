...
func TestIPPoolTool_Pull(t *testing.T) {
	...
	ip, err = ipPool.Pull()
	require.NoError(t, err)
	require.Equal(t, ip.String(), "192.0.0.1")
...
func TestIPPoolTool_PullP2PAddrs(t *testing.T) {
	...
	require.Equal(t, dstIPNet.String(), "192.0.0.1/32")

/*	srcIPNet, err = ipPool.PullWithExclude()
	require.NoError(t, err)
	require.Equal(t, srcIPNet.String(), "192.0.0.2/32")*/
