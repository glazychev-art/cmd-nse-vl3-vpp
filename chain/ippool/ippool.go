...
// PullWithExclude - returns next IP address excluding specified
func (tree *IPPool) PullWithExclude(exclude ...*IPPool) (ipNet *net.IPNet, err error) {
	tree.lock.Lock()
	defer tree.lock.Unlock()
	...
	ipNet = &net.IPNet{
		IP:   ipFromIPAddress(ip, tree.ipLength),
